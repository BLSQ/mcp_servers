...
def get_organisation_units(
    ...
    meta = dhis2.meta.organisation_units(
        fields="id,name,level,path,openingDate,closedDate,geometry", filters=filters
    )
    ...
    for row in levels.iter_rows(named=True):
        ...
        df = df.with_columns(
            ...
            .str.split("/")
            .list.slice(1)
            .list.get(lvl - 1, null_on_oob=True)
            .alias(f"level_{lvl}_id")
        )
...
def get_attributes(dhis2: DHIS2, filters: list[str] | None = None) -> pl.DataFrame:
    ...
    """
    r = dhis2.api.get(
        "attributes", params={"paging": False, "fields": "id,name", "filter": filters}
    )
    rows = r["attributes"]
    schema = {"id": str, "name": str}
...
def get_tracked_entity_types(dhis2: DHIS2, filters: list[str] | None = None) -> pl.DataFrame:
    ...
    """
    rows = []
    for page in dhis2.api.get_paged(
        "trackedEntityTypes", params={"fields": "id,name", "filter": filters}
    ):
